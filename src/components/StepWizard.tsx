...
มอบความรักชั่วนิรันดร์ ตลอดปีเทอญ`;
  ...
  const handleFinishAndSave = () => {
    ...
    // Proceed to donation screen
    setStep(9);
  };

  const handleSubmitToLine = () => {
    const blessingsLabel = selectedBlessings.join(", ");
    const specialLabel = selectedSpecialDetail ? selectedSpecialDetail.replace(/[\u1F600-\u1F64F\u1F300-\u1F5FF\u1F680-\u1F6FF\u2600-\u26FF\u2700-\u27BF]/g, "").trim() : "ทั่วไป";
    const summaryText = `🎵 สั่งแต่งเพลงสุนทรีย์ AI 🎵\nรหัสสั่งซื้อ: ${orderId}\nโอกาส: ${selectedOccasion}\nคำอวยพร: ${blessingsLabel}\nเรื่องพิเศษในเพลง: ${specialLabel}\nแนวเพลง: ${selectedGenre} (${selectedTempo})\nผู้รับ: ${selectedRel?.name} (${selectedNickname === "พูดชื่อเล่นเองใน LINE 🎙️" ? "พูดชื่อใน LINE" : selectedNickname})\nยอดสนับสนุน: ร่วมสมทบทุนตามพึงพอใจ`;
      ...
      <div className="step-container">
        ...
        {step === 8 && (
          <div className="step-fade-in" style={{ display: "flex", flexDirection: "column", gap: "1rem", height: "100%", justifyContent: "space-between" }}>
            <div className="step-header">
              <h2 className="step-title" style={{ color: "var(--primary)" }}>ใบสรุปรายการสั่งผลิตเพลงแสนสุข 🎁</h2>
              <p className="step-subtitle">ตรวจสอบความถูกต้องของเนื้อหาเพลงสั่งผลิตและข้อมูลผู้รับได้เลยค่ะ</p>
            </div>
              ...
              <div className="receipt-ticket-premium">
                ...
                <div className="ticket-details-premium">
                  <div className="ticket-row-premium">
                    <span className="ticket-label-premium">รหัสใบสั่งซื้อ:</span>
                  ...
                  <div className="ticket-row-premium">
                    <span className="ticket-label-premium">คำอวยพรเด่น:</span>
                    <span className="ticket-value-premium">
                      {selectedBlessings.map(b => b.replace(/[\u1F600-\u1F64F\u1F300-\u1F5FF\u1F680-\u1F6FF\u2600-\u26FF\u2700-\u27BF]/g, "").trim()).join(", ") || "ความสุขสมหวัง"}
                    </span>
                  </div>
                  <div className="ticket-row-premium">
                    <span className="ticket-label-premium">เรื่องราวพิเศษ:</span>
                    <span className="ticket-value-premium">
                      {selectedSpecialDetail ? selectedSpecialDetail.replace(/[\u1F600-\u1F64F\u1F300-\u1F5FF\u1F680-\u1F6FF\u2600-\u26FF\u2700-\u27BF]/g, "").trim() : "ทั่วไป"}
                    </span>
                  </div>
                  <div className="ticket-row-premium">
                    <span className="ticket-label-premium">สไตล์ดนตรี:</span>
                    <span className="ticket-value-premium">{selectedGenre} ({selectedTempo})</span>
                  </div>
                  <div className="ticket-row-premium">
                    <span className="ticket-label-premium">มอบให้:</span>
                    <span className="ticket-value-premium" style={{ fontWeight: "800", textDecoration: "underline" }}>
                      {selectedRel?.name} • {selectedNickname === "พูดชื่อเล่นเองใน LINE 🎙️" ? "อัดเสียงผ่าน LINE" : selectedNickname}
                    </span>
                  </div>
                </div>

                <div className="ticket-lyrics-preview">
                  <div className="lyrics-preview-title">ตัวอย่างคำร้องอวยพร (กลอนสุนทรีย์)</div>
                  <div className="lyrics-preview-body">{generatedLyrics}</div>
                </div>

                <div className="ticket-price-badge">
                  ร่วมสมทบทุนตามพึงพอใจ 💛
                </div>
              </div>
            </div>
            ...
            <div className="wizard-footer" style={{ borderTop: "none" }}>
              ...
              >
                ไปหน้าสนับสนุนโอนเงิน ➡️
              </button>
            </div>
          </div>
        )}

        {/* STEP 9: Clean PromtPay Donation Screen */}
        {step === 9 && (
          <div className="step-fade-in" style={{ display: "flex", flexDirection: "column", gap: "1.5rem", height: "100%", justifyContent: "space-between", alignItems: "center" }}>
            <div className="step-header" style={{ width: "100%", textAlign: "center" }}>
              <h2 className="step-title">ร่วมสนับสนุนน้ำใจนักพัฒนา 💖</h2>
              <p className="step-subtitle">หากชื่นชอบและอยากมีส่วนร่วมสนับสนุน สามารถอ่านข้อมูลด้านล่างได้เลยครับ</p>
            </div>

            <div className="donation-card-expanded">
              <div className="donation-heart-animated">💝</div>
              <p className="donation-main-text">
                "ถ้าชอบ อยากสนับสนุนผม สแกนโอนเงินได้นะครับ เท่าไหร่ก็ได้ เดี๋ยวจะส่งรูป qr code ให้"
              </p>
              <div className="donation-sub-badge">
                ร่วมสนับสนุนสนับสนุนผู้สร้างและส่งเสริมความสุขให้ผู้สูงอายุ
              </div>
            </div>

            <div className="wizard-footer" style={{ borderTop: "none", width: "100%" }}>
        ...
        {step === 10 && (
          <div className="step-fade-in" style={{ display: "flex", flexDirection: "column", gap: "1.5rem", height: "100%", justifyContent: "space-between", alignItems: "center" }}>
            <div className="step-header" style={{ width: "100%", textAlign: "center" }}>
              ...
              <p className="step-subtitle">ถ่ายรูปหน้าจอสรุปข้อมูล หรือส่งข้อมูลผ่านแอปไลน์เพื่อให้ผู้แต่งเริ่มงานแต่งดนตรีค่ะ</p>
            </div>

            <div className="line-instructions-box">
              <div className="instruction-item">
                <span className="item-num">1</span>
                <span className="item-desc"><strong>แคปรูปภาพหน้าจอ</strong> หน้านี้หรือหน้าสรุปข้อมูลไว้</span>
              </div>
              <div className="instruction-item">
                <span className="item-num">2</span>
                <span className="item-desc">แตะปุ่มสีเขียวด้านล่าง <strong>ระบบจะเปิดห้องแชตไลน์พร้อมคัดลอกรายละเอียดเพลงให้คุณตาคุณยายอัตโนมัติ</strong></span>
              </div>
              <div className="instruction-item">
                <span className="item-num">3</span>
                <span className="item-desc">ในห้องแชต ให้<strong>แตะค้างที่ช่องพิมพ์ข้อความแล้วกด 'วาง' (Paste)</strong> แล้วส่งให้เราได้ทันทีค่ะ!</span>
              </div>
            </div>
